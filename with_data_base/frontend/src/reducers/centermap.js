...
const centermap = (state = center, action) => {
  switch (action.type) {
    case "UPDATECENTER":
      if (
        action.searchkey.toUpperCase().trim() === "A" ||
        action.searchkey.toUpperCase().trim() === "K"
      ) {
        return { lat: 51, lng: -114, zoom: 3.3 }
      } else if (action.searchkey.toUpperCase().trim() === "AK") {
        return { lat: 63, lng: -147, zoom: 5 }
      } else if (
        action.searchkey.toUpperCase().trim() === "H" ||
        action.searchkey.toUpperCase() === "I"
      ) {
        return { lat: 36.5, lng: -115, zoom: 4 }
      } else if (action.searchkey.toUpperCase().trim() === "HI") {
